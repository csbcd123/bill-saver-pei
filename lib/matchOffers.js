...
function canonicalProvider(provider = "") {
  ...
  if (normalized.includes("koodointernet")) return "koodointernet";
  return normalized;
}
...
function getCurrentPrice(profile, serviceType) {
  if (serviceType === "internet") {
    return Number(profile.internet_monthly_price || 0);
  }

  if (serviceType === "mobile") {
    return Number(profile.mobile_monthly_price || 0);
  }
...
function getCurrentProvider(profile, serviceType) {
  return serviceType === "internet" ? profile.internet_provider : profile.mobile_provider;
}
...
export function matchOffers(profile) {
  ...
  return offers
    ...
    .map((offer) => {
      ...
      if (profile.willing_to_switch === "maybe") score += 8;
      ...
      if (
        ...
        score -= 15;
        strategyOnly = true;
        reasons.push("当前是 Bell/Bell Aliant，winback/隐藏价可能需要离网或人工确认，只能作为谈判策略。");
      }
