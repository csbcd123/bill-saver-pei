...
export function matchOffers(profile) {
  ...
  return offers
    ...
    .filter((offer) => purpleCowUsageMatches(offer, profile))
    ...
    .map((offer) => {
      ...
      const recommendationType =
        meetsServiceLevel && annualSaving >= 150
          ? "best_savings"
          : meetsServiceLevel && annualSaving > 0
            ? "small_savings"
            : meetsServiceLevel && betterServiceLevel
              ? "upgrade_option"
              : "low_priority";
    ...
    .sort((a, b) => {
      const typeRank = { best_savings: 1, small_savings: 2, upgrade_option: 3 };
